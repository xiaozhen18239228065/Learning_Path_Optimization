...
#    global MAX
#    for i in range(MAX):
#        f=codecs.open("./group/group"+str(i),"r","utf-8")
#        for line in f:
#            line=line.strip()
#            if line=="":continue
...
def read_wiki_data():
    global wiki_data
    f=codecs.open("uniq_data.txt","r","utf-8")
    for line in f:
        line=line.strip()
        if line=="":continue
        idx=line.find('#')
...
#            return i
#    return -1
def caculate_steps(start_word,end_word):
    global wiki_data
    MAX_DEEP=6
    visit=set()
...
#        start_time=time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))
#        print(caculate_steps(start_word,end_word))
#        end_time=time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))
#        print(start_time+"---->"+end_time)
